...
/* Neutraliza cualquier fondo interno (el círculo del botón "borrar todo",
   separadores, etc.) para que solo queden los dos colores que sí queremos:
   la caja general (rosado) y las píldoras (rojo). La regla de la píldora
   de abajo tiene mayor especificidad, así que sigue ganando su color. */
[data-testid="stSidebar"] .stMultiSelect [data-baseweb="select"] * {
    background-color: transparent !important;
    box-shadow: none !important;
}
[data-testid="stSidebar"] .stMultiSelect span[data-baseweb="tag"] {
    background-color: #a33a52 !important;
...
/* El menú desplegable del multiselect (lista de opciones) se pinta
   fuera del sidebar en el DOM, así que se estiliza aparte para que
   combine con la paleta rosada en vez del look genérico por defecto. */
div[data-baseweb="popover"] ul[data-baseweb="menu"] {
    border: 1px solid rgba(200,92,114,0.35) !important;
}
div[data-baseweb="popover"] ul[data-baseweb="menu"] li {
    background-color: #fff !important;
}
div[data-baseweb="popover"] ul[data-baseweb="menu"] li:hover {
    background-color: rgba(200,92,114,0.15) !important;
}
div[data-baseweb="popover"] ul[data-baseweb="menu"] li[aria-selected="true"] {
    background-color: rgba(200,92,114,0.25) !important;
}
...
/* Ocultar el cursor de texto (caret) dentro de los selectbox: siguen
   permitiendo escribir para filtrar, pero visualmente no muestran la
   línea parpadeante de "campo de texto". */
.stSelectbox input,
.stMultiSelect input {
...
# ------------------------------------------------
# EVITAR QUE SE ABRA EL TECLADO EN MÓVIL AL TOCAR UN SELECTBOX
# (el input interno de los selectbox permite escribir para filtrar;
# en celulares eso dispara el teclado táctil, que aquí no queremos.
# inputmode="none" mantiene el campo tocable/enfocable pero le pide
# al navegador que no muestre el teclado virtual.)
# ------------------------------------------------
components.html(
    """
    <script>
    function ocultarTecladoSelects() {
        try {
    ...
    ocultarTecladoSelects();
    const obs = new MutationObserver(ocultarTecladoSelects);
...
ciclos_sel = st.sidebar.multiselect(
    "Ciclo(s)", ciclos_ordenados,
    default=[ciclos_ordenados[0]] if ciclos_ordenados else [],
)
plan    = st.sidebar.selectbox("Plan de estudios", sorted(df["plan de estudios"].dropna().unique()))

if not ciclos_sel:
    st.sidebar.warning("Selecciona al menos un ciclo.")
    st.stop()

ciclo_txt = ", ".join(ciclos_sel)

filtrado_base = df[
    (df["carrera"]==carrera) & (df["ciclo"].isin(ciclos_sel)) & (df["plan de estudios"]==plan)
]
...
if filtrado_base.empty:
    st.warning(f"No hay cursos para {carrera} — {ciclo_txt} — Plan {plan}.")
    st.stop()
if filtrado.empty:
    st.warning(f"No hay cursos en la sede **{sede}** para {carrera} — {ciclo_txt} — Plan {plan}.")
    st.stop()
...
if modo_actual == "simulador":
    ...
    st.caption(f"Se encontraron **{len(cursos)} cursos** para {carrera} — {ciclo_txt} — Plan {plan}.")
